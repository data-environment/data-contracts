...
1. Crie um arquivo em `definitions/<parceiro_ou_fonte>/<dataset>.py`.
2. Use [model/example.py](model/example.py) como ponto de partida — ele mostra o preenchimento de cada bloco (`General`, `DataSource`, `DataExtraction`, `DataSchema`, `DataQuality`, `DataIngestion`, `Distribution`, `Transformation`, `BusinessRules`) com comentários explicando o propósito de cada campo.
...
4. Exporte o `DataContract` resultante em [definitions/registry.py](definitions/registry.py).
